...
import { Search, X, GraduationCap, FileText, Newspaper, ArrowRight } from 'lucide-react';
...
export function SearchOverlay({ isOpen, onClose }: SearchOverlayProps) {
    ...
    return (
      <div key={category} className="mb-2">
        ...
        <div className="flex items-center gap-2 px-5 py-2">
          <Icon size={14} className="text-primary dark:text-primary-light shrink-0" />
          <span className="text-text-secondary-light dark:text-text-secondary-dark text-xs font-semibold tracking-wide uppercase">
            {label}
          </span>
          ...
          return (
            ...
            >
              <div className="min-w-0 flex-1">
                <p className="text-text-primary-light dark:text-text-primary-dark truncate text-sm font-medium">
                  {item.title}
                </p>
                <p className="text-text-secondary-light dark:text-text-secondary-dark mt-0.5 truncate text-xs">
                  {item.description}
                </p>
              </div>
              <ArrowRight
                size={14}
                className="text-text-secondary-light/40 dark:text-text-secondary-dark/40 shrink-0"
              />
            </Link>
          );
        })}
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-start justify-center pt-32">
      ...
      {/* Dialog */}
      <div className="bg-surface-light dark:bg-surface-dark animate-in fade-in slide-in-from-top-4 relative mx-4 w-full max-w-2xl overflow-hidden rounded-xl shadow-2xl duration-300">
        {/* Search input row */}
        <div className="border-border-light/50 dark:border-border-dark/50 flex items-center gap-3 border-b px-5 py-4">
          <Search
            size={20}
          ...
          <input
            ...
            className="text-text-primary-light dark:text-text-primary-dark placeholder:text-text-secondary-light/60 dark:placeholder:text-text-secondary-dark/60 flex-1 bg-transparent text-lg outline-none"
          />
          <button
            onClick={onClose}
        ...
        {/* Footer hint */}
        <div className="border-border-light/50 dark:border-border-dark/50 text-text-secondary-light/60 dark:text-text-secondary-dark/60 flex items-center justify-between border-t px-5 py-2.5 text-[11px]">

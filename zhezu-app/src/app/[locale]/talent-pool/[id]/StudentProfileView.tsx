  ...
  return (
    <div className="min-h-screen bg-bg-dark">
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-8 pb-20">
        {/* ── Breadcrumb ── */}
        <nav className="flex items-center gap-2 text-sm text-text-secondary-dark mb-8 flex-wrap">
          <Link href="/" className="hover:text-white transition-colors">{tNav('home')}</Link>
          <ChevronRight size={14} className="text-text-secondary-dark/40" />
          <Link href="/talent-pool" className="hover:text-white transition-colors">{t('title')}</Link>
          <ChevronRight size={14} className="text-text-secondary-dark/40" />
          <span className="text-white/60">{student.major[locale]}</span>
          <ChevronRight size={14} className="text-text-secondary-dark/40" />
        ...
        <div className="premium-card p-7 md:p-10 mb-6">
          <div className="flex flex-col md:flex-row gap-6 md:gap-8 items-center md:items-start">
            ...
            <div className="relative shrink-0">
              <div className="w-32 h-32 md:w-36 md:h-36 rounded-full overflow-hidden ring-4 ring-gold/30 ring-offset-4 ring-offset-[#0B1121]">
                <Image
                  src={student.photo}
                  alt={student.name}
                  width={144}
              ...
              <div className="absolute -bottom-1 -right-1 w-10 h-10 rounded-full bg-gradient-to-br from-gold to-gold-light flex items-center justify-center shadow-lg shadow-gold/30">
                <Star size={18} className="text-bg-dark fill-bg-dark" />
              </div>
            </div>
            ...
            <div className="flex-1 min-w-0 text-center md:text-left">
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 mb-2">
                ...
                <span className="px-3 py-1 rounded-full bg-gold/15 border border-gold/30 text-[10px] font-bold text-gold uppercase tracking-[0.15em]">
                  {t('topTalent')}
                </span>
              </div>
              ...
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-x-5 gap-y-2 mt-3 text-sm text-text-secondary-dark">
                <span className="flex items-center gap-1.5">
                  <Building2 size={14} className="text-text-secondary-dark/60" />
                  {t('universityName')}
                </span>
                <span className="flex items-center gap-1.5">
                  <MapPin size={14} className="text-text-secondary-dark/60" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="premium-card p-7">
              <div className="flex items-center justify-between mb-5">
                ...
                <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-primary/8 border border-primary/15 text-[10px] font-semibold text-primary-light/70">
                  <Sparkles size={10} />
                  {t('aiGenerated')}
                </span>
              </div>
              <p className="text-sm text-text-primary-dark leading-[1.85]">
                      ...
                      <div key={comp.name.en} className="space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-bold text-text-secondary-dark tabular-nums">
                              {comp.percentage}%
                            </span>
                          </div>
                        </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {student.achievements.map((ach, idx) => {
                  ...
                  return (
                    <div
                      key={ach.id}
                      className="p-5 rounded-xl bg-surface-elevated-dark/30 border border-border-dark/25 hover:border-opacity-50 transition-colors duration-300 text-center"
                    >
                      <div
                      ...
                      <p className="text-xs text-text-secondary-dark">{ach.issuer[locale]}</p>
                      <p className="text-xs text-text-secondary-dark/50 mt-2 flex items-center justify-center gap-1">
                        <Calendar size={10} />
                        {ach.date}
                      </p>
                    </div>
              ...
              <div className="premium-card p-7">
                ...
                <div className="space-y-4">
                  {student.portfolio.map((item) => (
                    <div
                      key={item.id}
                      className="flex flex-col sm:flex-row items-start gap-4 p-5 rounded-xl bg-surface-elevated-dark/30 border border-border-dark/25 group hover:border-primary/20 transition-all duration-300"
                    >
                      ...
                      <div className="w-full sm:w-24 h-20 sm:h-16 rounded-lg bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/10 flex items-center justify-center shrink-0">
                        <FileText size={20} className="text-primary-light/50" />
                      </div>
                        ...
                        <div className="mt-3 flex items-center gap-4">
                          {item.link && (
                            <a
                              ...
                              className="text-xs text-primary-light font-medium flex items-center gap-1 hover:underline"
                            >
                          ...
                          <span className="text-[10px] font-bold text-text-secondary-dark/40 uppercase tracking-wider">
                            {item.type}
                          </span>
                        </div>
                      </div>
          ...
          <div className="space-y-6">
            ...
            <div className="premium-card premium-card-gold p-7">
              <h2 className="text-lg font-display font-bold text-white mb-1 flex items-center gap-3">
                <div className="w-1 h-6 rounded-full bg-gradient-to-b from-gold-light to-gold" />
                {t('recruiterActions')}
              </h2>
              <p className="text-xs text-text-secondary-dark mb-5 ml-4">
                {t('officialPortal')}
              </p>
              ...
              <div className="space-y-3">
                ...
                <button className="w-full flex items-center justify-center gap-2.5 py-3.5 px-4 rounded-xl bg-surface-elevated-dark/50 border border-border-dark/50 text-white text-sm font-semibold hover:border-border-dark transition-all cursor-pointer">
                  <Download size={16} />
                  {t('downloadCV')}
                </button>
                <button className="w-full flex items-center justify-center gap-2.5 py-3.5 px-4 rounded-xl bg-gold/6 border border-gold/20 text-gold text-sm font-semibold hover:bg-gold/12 hover:border-gold/30 transition-all cursor-pointer">
                  <Send size={16} />
                  {t('sendOffer')}
                </button>
              </div>
              ...
              {/* Privacy note */}
              <div className="mt-5 pt-5 border-t border-border-dark/25">
                <p className="text-[10px] text-text-secondary-dark/50 flex items-start gap-2 leading-relaxed">
            ...
            <div className="premium-card p-7">
              ...
              <div className="space-y-3">
                {similarStudents.map((s) => (
                  ...
                  >
                    <div className="relative w-11 h-11 rounded-full overflow-hidden ring-2 ring-border-dark/30">
                      <Image
                        src={s.photo}
                        alt={s.name}
                        fill
                    ...
                    <div className="flex flex-col items-end gap-0.5">
                      <div className="text-xs font-bold text-gold tabular-nums">{s.gpa}</div>
                      <ChevronRight size={14} className="text-text-secondary-dark/30" />
                    </div>
                  </Link>
                ))}
              </div>
              ...
              <Link
                href="/talent-pool"
                className="mt-4 w-full flex items-center justify-center gap-2 py-3 rounded-xl border border-border-dark/30 text-sm text-text-secondary-dark font-medium hover:text-white hover:border-border-dark/60 transition-all"
              >
                {t('viewAllCandidates')}

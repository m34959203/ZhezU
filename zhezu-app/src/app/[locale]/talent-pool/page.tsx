    ...
    <div className="min-h-screen bg-bg-dark">
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto">
            ...
            <div className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-gold/6 border border-gold/15 mb-8 backdrop-blur-sm">
              ...
              <span className="text-xs font-semibold text-gold uppercase tracking-widest">{t('badge')}</span>
            </div>

            <h1 className="text-5xl md:text-6xl lg:text-7xl font-display font-bold text-gradient mb-5 leading-[1.1] tracking-tight">
              {t('title')}
            </h1>
            <p className="text-lg md:text-xl text-text-secondary-dark max-w-2xl mx-auto mb-10 leading-relaxed">
              {t('subtitle')}
            </p>
            ...
            <div className="relative max-w-2xl mx-auto group">
              ...
              <div className="relative">
                <Search size={20} className="absolute left-5 top-1/2 -translate-y-1/2 text-text-secondary-dark/60" />
                <input
                  type="text"
                  placeholder={t('searchPlaceholder')}
                  className="w-full pl-13 pr-28 py-4 rounded-2xl bg-surface-dark/60 border border-border-dark/50 text-white placeholder:text-text-secondary-dark/50 focus:outline-none focus:border-primary/40 focus:bg-surface-dark/80 backdrop-blur-md transition-all duration-300 text-sm"
                />
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-16 max-w-4xl mx-auto">
            {stats.map((stat) => (
              ...
              >
                ...
                <span className="text-[11px] text-text-secondary-dark uppercase tracking-wider font-medium">{t(stat.labelKey)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-6">
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-xs text-text-secondary-dark mr-2 uppercase tracking-wider font-medium">{t('filterBy')}:</span>
          {['Mining', 'Metallurgy', 'Energy', 'Law', 'Engineering', 'IT'].map((chip) => (
            <button
              key={chip}
              className="px-3.5 py-1.5 rounded-full text-[11px] font-semibold border border-border-dark/40 text-text-secondary-dark hover:border-gold/30 hover:text-gold hover:bg-gold/5 transition-all duration-200 cursor-pointer"
            >
              {chip}
            </button>
          ))}

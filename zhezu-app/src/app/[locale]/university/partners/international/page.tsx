...
export default function InternationalPartnersPage() {
  ...
  const directions = [
    { key: 'exchange', icon: Users, color: 'text-primary dark:text-primary-light', bg: 'bg-primary/10 dark:bg-primary-light/10' },
    { key: 'research', icon: BookOpen, color: 'text-gold', bg: 'bg-gold/10' },
    { key: 'dual', icon: GraduationCap, color: 'text-success', bg: 'bg-success/10' },
    { key: 'grants', icon: Handshake, color: 'text-purple-600 dark:text-purple-400', bg: 'bg-purple-600/10 dark:bg-purple-400/10' },
  ];
  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="bg-surface-light dark:bg-surface-dark/50 py-16 lg:py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="font-display mb-3 text-center text-3xl font-bold">{t('directionsTitle')}</h2>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {directions.map((d) => (
              <Card key={d.key} padding="lg" hover>
                <div className={`mb-4 flex h-12 w-12 items-center justify-center rounded-xl ${d.bg}`}>
                  <d.icon size={24} className={d.color} />
                </div>
                <h3 className="font-display mb-2 text-base font-semibold">{t(`directions.${d.key}.title`)}</h3>

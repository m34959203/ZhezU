...
| FR-012 | Фасетная фильтрация: уровень (бакалавриат/магистратура), кафедра, язык обучения, форма | Must | 2 | Частично |
| FR-013 | Полнотекстовый поиск по названию и описанию программ (Meilisearch) | Should | 2 | Частично (клиентский поиск) |
| FR-014 | Детальная страница программы: описание, учебный план, карьерные перспективы, преподаватели | Must | 2 | ✅ (/academics/[slug]) |
...
| FR-019 | Связь с профилями преподавателей кафедры | Should | 3 | ✅ (/academics/faculty) |
...
| ID | Требование | Приоритет | Фаза |
|----|-----------|:---------:|:----:|
| FR-021 | Многошаговая форма подачи заявки (Form Wizard: 5+ шагов) | Must | 3 | Частично (/admission/apply — страница есть) |
...
| FR-025 | Отслеживание статуса заявки (timeline) | Must | 3 | ✅ (/admission/status — страница есть) |
| FR-026 | Email-уведомления при смене статуса (SendGrid) | Must | 3 | — |
| FR-027 | Калькулятор стипендий | Should | 3 | Частично (/admission/scholarships) |
| FR-028 | Чек-лист документов по выбранной программе | Must | 3 | ✅ (/admission/documents) |
| FR-029 | FAQ с accordion и поиском (CMS-управляемый) | Should | 2 | ✅ (/admission/faq с Accordion) |
| FR-030 | Виртуальный тур по кампусу (360° панорамы) | Should | 4 | — |
| FR-031 | Административная панель: список заявок, фильтры, статистика воронки | Must | 3 | Частично (админ-панель с CRUD новостей) |
...
| ID | Требование | Приоритет | Фаза |
|----|-----------|:---------:|:----:|
| FR-033 | Лента новостей с категоризацией (университет, наука, студенты, спорт) | Must | 2 | ✅ (9 категорий, API + админ-панель) |
| FR-034 | Детальная страница новости с rich-text (CMS), изображения, видео | Must | 2 | ✅ (/life/news/[id]) |
| FR-035 | Календарь событий (month/week/list view) + интеграция Google Calendar | Should | 3 | Частично (страницы есть, нет Google Calendar) |
...
| FR-040 | Тегирование и связанные новости (related articles) | Should | 2 | Частично (категории есть, related — нет) |

### 3.5 Глобальный поиск (FR-041 — FR-046)

| ID | Требование | Приоритет | Фаза |
|----|-----------|:---------:|:----:|
| FR-041 | Поисковая строка в header с автодополнением (typeahead) | Must | 2 | Частично (SearchOverlay есть) |
| FR-042 | Полнотекстовый поиск по всему контенту (Meilisearch) | Must | 2 | — (нет Meilisearch) |
...
| FR-058 | Locale-detection по Accept-Language + GeoIP (fallback: ru) | Should | 2 | ✅ (Accept-Language через middleware) |
| FR-059 | RTL-поддержка (потенциально для арабского в будущем) | Won't | — | — |
| FR-060 | Локализованные форматы дат, чисел, валют (Intl API) | Should | 2 | Частично |
...
| ID | Требование | Приоритет | Фаза |
|----|-----------|:---------:|:----:|
| FR-069 | Витрина AI-проектов (Card Grid с фильтрами по типу, технологии, статусу) | Should | 4 | ✅ (/ai-center/projects, /ai-center/showcase) |
| FR-070 | Каталог AI-агентов: описание, демо, API-доступ | Should | 4 | ✅ (/ai-center/agents/assistant, /agents/talapker) |
| FR-071 | Каталог AI-приложений: веб-инструменты, калькуляторы, генераторы | Should | 4 | Частично (/ai-center — страница есть) |
| FR-072 | Каталог AI-инструментов (tools): datasets, libraries, frameworks | Could | 5 | ✅ (/ai-center/tools) |
| FR-073 | AI-лаборатория: sandbox для экспериментов | Could | 6 | ✅ (/ai-center/lab, /ai-center/lab/join) |
| FR-074 | AI-чатбот для абитуриентов (FAQ, навигация, рекомендации программ) | Should | 3 | — (нет AI-интеграции) |

### 3.10 Карьерный центр и пул талантов (FR-075 — FR-082)

| ID | Требование | Приоритет | Фаза |
|----|-----------|:---------:|:----:|
| FR-075 | Публичные профили студентов (портфолио, навыки, GPA, проекты) | Should | 4 | ✅ (/talent-pool/[id]) |
| FR-076 | Фильтрация студентов: кафедра, навыки, GPA, курс, язык | Should | 4 | Частично (/talent-pool) |
| FR-077 | Вакансии и стажировки (CMS-управляемые карточки) | Should | 4 | ✅ (/career — страница есть) |
...
| FR-080 | Система Talapker: геймификация (20 уровней, XP, квесты, значки) | Could | 4-5 | ✅ (данные + UI готовы, нет backend) |
